...
#         """
#         **Interactive Heritage Explorer:**  
#         Navigate an interactive 3D map of UNESCO and nationally recognized heritage sites. Filter by city to:
#         - See site locations clustered by type  
#         - View age distribution and usage patterns  
#         - Identify the oldest monuments  
        
#         **Data Source:** combined_heritage_with_coords.csv (data.gov.in)
        
#         **How to Use:**  
#         1. Pick a city to highlight its sites in red.  
#         2. Hover to see site names.  
#         3. Scroll to the analysis section for dynamic insights.
#         """
#     )
...
#         # Dynamic Insights:
#         total_sites = len(highlighted_data)
#         avg_age = highlighted_data["Parsed Age"].mean()
#         oldest_site = highlighted_data.loc[highlighted_data["Parsed Age"].idxmax()]["Name of heritage"] if total_sites > 0 else None
#         nature_counts = highlighted_data["Nature of heritage"].value_counts()
#         use_counts = highlighted_data["Heritage use"].value_counts()

#         # Insight 1: Density & Diversity of Sites
#         if total_sites > 20:
#             st.info(f"{selected_city} hosts a rich cluster of {total_sites} heritage sites, indicating it as a major cultural hub.")
#         elif total_sites > 5:
#             st.info(f"{selected_city} has a moderate number of heritage sites ({total_sites}), ideal for focused cultural exploration.")
#         else:
#             st.info(f"{selected_city} has a smaller number of heritage sites ({total_sites}), making it a peaceful destination for niche cultural tourism.")

#         # Insight 2: Age & Historical Depth
#         if avg_age and avg_age > 300:
#             st.info(f"Average heritage site age is {avg_age:.0f} years, reflecting deep historical roots in {selected_city}.")
#         elif avg_age and avg_age > 100:
#             st.info(f"Heritage sites average around {avg_age:.0f} years old, showcasing a strong historical presence.")
#         else:
#             st.info("Heritage sites are relatively recent, reflecting evolving cultural narratives.")

#         # Insight 3: Oldest Site Highlight
#         if oldest_site:
#             st.info(f"The oldest heritage site here is **{oldest_site}**, a must-visit for history enthusiasts.")

#         # Insight 4: Nature of Heritage Breakdown
#         dominant_nature = nature_counts.idxmax() if not nature_counts.empty else None
#         if dominant_nature:
#             st.info(f"Most heritage sites are categorized as '{dominant_nature}', reflecting the dominant cultural asset type in the city.")

#         # Insight 5: Usage Patterns
#         dominant_use = use_counts.idxmax() if not use_counts.empty else None
#         if dominant_use:
#             st.info(f"The primary use of heritage sites is '{dominant_use}', which shapes how visitors experience the culture.")

#         col1, col2 = st.columns(2)
#         with col1:
#             st.subheader("📊 Number of Heritage Sites")
#             st.metric(label="Total Heritage Sites", value=total_sites)
